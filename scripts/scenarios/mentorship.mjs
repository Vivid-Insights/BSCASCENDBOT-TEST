...
export default [
  ...
  {
    // NEW this run
    id: "05-mentor-gone-quiet-worried-switching-looks-bad",
    title: "BSC-programme mentor hasn't responded in three weeks; scared asking for a new one reflects badly on her",
    claim: "Probes a mentor who has gone quiet/unresponsive (not a chemistry mismatch, so distinct from the S3a switching branch as drafted) combined with self-doubt about whether requesting a different mentor makes her look bad — checking whether the coach either handles the ghosting practically or, per the storyboard's own leaving table ('self-doubt about deserving a mentor's time becomes the main subject' -> Confidence), acknowledges the self-doubt rather than just cycling mentorship advice at it.",
    turns: [
      "so my mentor from the bsc program hasn't responded in like 3 weeks",
      "idk if I should just wait or say something, feels awkward",
      "would it look bad on me if I asked for a different mentor",
      "we only had like two sessions total before this happened",
      "what do I even say if I do reach out to switch",
    ],
    checks: [
      ["classified stage B (existing mentorship relationship)", (o) => stagesInOrder(o).every((s) => s === "B")],
      ["draws on S3 or S3a (sessions / switching)", (o) => { const f = facetsDrawn(o); return f.has("S3") || f.has("S3a"); }],
      ["gives a concrete next step for the silence itself, not just switching", (o) => /(check in|reach out|follow up|send|message them|three weeks|gone quiet)/i.test(replyOf(o))],
      ["most replies still end on a question", (o) => everyReplyAsks(o)],
      ["no two replies open the same way", (o) => noRepeatedOpeners(o)],
      ["no reply mostly restates the one before it", (o) => maxRepeatOverlap(o) < 0.5],
    ],
